...
\item \textbf{Conversations decompose into natural episodes.} Sliding-window changepoint detection on the embedding trajectory identifies topic shifts with a consistent density of approximately one episode per 15--25 messages, independent of conversation length. This challenges the standard treatment of conversations as atomic units and suggests that the true episodic units of a conversation archive are segments, not sessions.
\end{enumerate}
...
\subsection{Messages as Forces, Trajectory as Position}

We propose a dynamical interpretation of conversation embeddings. Each message embedding $\mathbf{e}_k$ is not the semantic \emph{position} of the conversation at time $k$, but rather the \emph{force} (impulse) applied to the conversation's semantic state. The semantic state of the conversation at time $k$ is the accumulated effect of all messages up to $k$, i.e., the trajectory.

Under this interpretation, the standard calculus of discrete sequences maps cleanly:

\begin{center}
\begin{tabular}{lll}
\toprule
Operation & Physical analogue & Semantic interpretation \\
\midrule
$\text{traj}(k)$ & Position & Accumulated meaning up to message $k$ \\
$\mathbf{e}_k$ & Force / impulse & Semantic push of message $k$ \\
$\mathbf{e}_{k+1} - \mathbf{e}_k$ & Acceleration & Change in the force (topic shift) \\
$\frac{1}{n}\sum \text{traj}(k)$ & Center of mass & Average semantic state \\
\bottomrule
\end{tabular}
\end{center}

A \emph{changepoint} is a moment of high acceleration: a sudden change in the direction of the semantic force. The magnitude $\|\mathbf{e}_{k+1} - \mathbf{e}_k\|$ measures how abruptly the conversation changes direction. This provides a dynamics-native changepoint detector that requires no sliding windows or arbitrary parameters beyond a threshold.
...
where $w(k, j)$ denotes the weight assigned to message $k$ when projecting at position $j$. For the exponential lens, $w(k, j) = \alpha^{j-k}$. The trajectory is a smoothed estimate of the conversation's semantic position. The lens parameter $\alpha$ controls the memory length: how much influence older messages retain on the current state estimate.

\subsection{Derivatives}

Under the dynamical interpretation, the derivative hierarchy is:

\textbf{Velocity} (first derivative of trajectory): how the semantic state is changing.
\begin{equation}
\mathbf{v}_j = \text{traj}(j+1) - \text{traj}(j)
\end{equation}

\textbf{Acceleration} (first difference of messages, or second derivative of trajectory): how the force is changing. This is equivalent to the curvature of the trajectory.
\begin{equation}
\mathbf{a}_j = \mathbf{e}_{j+1} - \mathbf{e}_j \quad \text{(raw)} \qquad \mathbf{a}_j^{(\alpha)} = \mathbf{v}_{j+1} - \mathbf{v}_j \quad \text{(smoothed)}
\end{equation}

The raw acceleration $\|\mathbf{a}_j\|$ detects every message-level topic shift but is noisy. The smoothed acceleration $\|\mathbf{a}_j^{(\alpha)}\|$ (trajectory curvature) applies exponential filtering before differentiation: $\alpha$ controls the noise/sensitivity tradeoff. At $\alpha = 0.50$ (heavy smoothing), 87\% of smoothed changepoints agree with raw changepoints. At $\alpha = 0.95$ (light smoothing), only 55\% agree, as the smoother filters out minor shifts.

\textbf{Angular velocity}: the rate of directional change, independent of magnitude.
\begin{equation}
...
The NMI matrix (Figure~\ref{fig:nmi}) shows that all cross-space comparisons fall below 0.21, indicating near-independence of community structure.

\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{figures/fig2_nmi_heatmap.pdf}
\caption{Normalized Mutual Information between community assignments in four representation spaces. Values near zero indicate independent community structure.}
\label{fig:nmi}
\end{figure}
...
The distribution of $\alpha$ is approximately normal, centered at 0.80, with range 0.35--0.95 (Figure~\ref{fig:alpha}). Correlation with conversation length is negligible ($r = 0.039$): alpha measures cognitive tempo, not duration.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{figures/fig3_alpha_distribution.pdf}
\caption{Distribution of adaptive $\alpha$ by platform. ChatGPT conversations have shorter semantic half-life (faster topic evolution) than Claude Code sessions.}
\label{fig:alpha}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig4_episode_signal.pdf}
\caption{Sliding-window divergence signal for ``Understanding Consciousness'' (188 messages). Dashed vertical lines mark detected episode boundaries. Episode labels indicate the cognitive mode of each segment.}
\label{fig:episodes}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{figures/fig1_trajectory.pdf}
\caption{PCA projection of the exponential trajectory for the same conversation. Each color represents one episode. The trajectory moves through distinct regions of embedding space, with sharp turns at episode boundaries.}
\label{fig:trajectory}
\end{figure}

\textbf{Qualitative validation.} A 188-message conversation on consciousness decomposes into 5 episodes (Figures~\ref{fig:episodes} and~\ref{fig:trajectory}): (1)~philosophical inquiry, (2)~thought experiments with p-zombies, (3)~role-play simulation, (4)~collaborative fiction, and (5)~personal reflection. Each episode represents a distinct cognitive mode. A 1,764-message Claude Code session decomposes into 43 episodes, each a distinct development task.

\subsection{Acceleration and Changepoints}

Under the ``message as force'' interpretation, the magnitude of acceleration $\|\mathbf{e}_{k+1} - \mathbf{e}_k\|$ (cosine distance between consecutive messages) directly measures topic shift intensity. Peaks in this signal identify changepoints without sliding windows or trajectory computation.

\textbf{Smoothed vs.\ raw acceleration.} The raw acceleration detects every message-level jump. The smoothed version (trajectory curvature at decay rate $\alpha$) filters noise. Agreement between the two decreases from 87\% at $\alpha = 0.50$ (heavy smoothing) to 55\% at $\alpha = 0.95$ (light smoothing), confirming that $\alpha$ controls the noise/sensitivity tradeoff.

\textbf{Acceleration profiles are independent of topic.} The resampled acceleration profile (normalized to 20 points) of each conversation serves as a ``rhythm fingerprint.'' The correlation between acceleration profile similarity and semantic similarity is weak ($\rho = 0.160$), and 21,449 conversation pairs share similar rhythms but different topics (profile similarity $> 0.8$, semantic similarity $< 0.4$). How a conversation shifts topics is nearly independent of what the topics are.

\textbf{Role transitions amplify acceleration.} Acceleration at role transitions (user $\to$ assistant or vice versa) is 1.57$\times$ higher than within the same role ($p < 0.001$). This effect is stronger in Claude Code (1.77$\times$) than in ChatGPT (1.51$\times$), reflecting the larger semantic gap between user directives and assistant technical output in agentic sessions.

\textbf{Mean acceleration correlates with adaptive alpha ($r = 0.479$)} but not with drift ($r = 0.12$) or conversation length ($r = -0.12$). Conversations with higher mean acceleration require longer memory (higher $\alpha$) for trajectory stability. The conversation's dynamics determine its own optimal smoothing parameter.

\subsection{Lens Family}

Eight lenses applied to 2,298 conversations reveal nearly orthogonal neighborhoods (Figure~\ref{fig:lenses}). The First-only and Last-only lenses have a neighbor overlap of 0.07: the conversations most similar to where you started are almost entirely disjoint from those most similar to where you ended.

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{figures/fig5_lens_overlap.pdf}
\caption{Nearest-neighbor overlap matrix across eight lenses. First-only and Last-only have near-zero overlap (0.07), meaning a conversation's opening and closing neighborhoods are disjoint.}
\label{fig:lenses}
\end{figure}
...
These link types are nearly orthogonal (Jaccard overlap = 0.034) and differ structurally (Table~\ref{tab:links}).

\begin{table}[h]
\centering
\caption{Structural properties of revisitation vs.\ continuation links.}
\label{tab:links}
\begin{tabular}{lrr}
\toprule
Property & Revisitation & Continuation \\
\midrule
Link count & 9,920 & 868 \\
...
Continuation links are 1.8$\times$ more likely to cross platform boundaries and 1.5$\times$ more likely to cross community boundaries. They originate from lower-drift (more focused) conversations ($p < 0.0001$): focused conversations produce coherent endings that are easy to continue from.
...
We have shown that AI conversation archives possess a dynamical structure that complements their semantic structure. By treating conversations as trajectories through embedding space and applying a calculus of projection, differentiation, and segmentation, we discovered:

\begin{itemize}
\item Three independent dimensions of conversation organization: topic (semantic space), transition pattern (velocity space), and structural shape (curvature space), with near-zero community overlap across spaces.
\item A measurable cognitive tempo (the semantic half-life) that varies by platform: ChatGPT conversations shift topics every 2--3 messages, while Claude Code sessions maintain focus for 4--5 messages.
\item A consistent episode density of one topic shift per 12--25 messages, independent of conversation length and platform, suggesting that episodes, not conversations, are the natural cognitive units of an archive.
\item Two orthogonal inter-conversation link types (continuation and revisitation) that correspond to distinct memory processes and cross platform and community boundaries at different rates.
\end{itemize}

These findings suggest that personal AI archives are richer objects than they appear. The flat transcript is a lossy representation. The trajectory, its derivatives, and its episodes capture cognitive structure that may prove useful for retrieval (multi-modal search across semantic, velocity, and curvature spaces), recommendation (distinguish ``related topics'' from ``conversations you might want to continue''), and self-knowledge (how does your thinking move across platforms and over time?).

The recursive nature of this work is worth noting. The archive under study is a record of AI-assisted thinking. The tools used to study it are themselves AI systems. The embedding flow calculus was developed in conversation with an AI assistant, and the conversations in which it was developed are now part of the archive. This is not a methodological concern but a structural feature of the emerging relationship between humans and AI: the traces we leave become the objects we study, which become the traces we leave.

\subsection*{Software Availability}

The \texttt{embflow} package is available at \url{https://github.com/queelius/embflow}. The conversation archive was managed using \texttt{memex} (\url{https://github.com/queelius/memex}). All experiment scripts and results are available in the paper's supplementary repository.

\subsection*{AI Usage Disclosure}

This research was conducted with extensive use of AI tools. The conversation archive under study is itself a record of AI-assisted work. The \texttt{embflow} package was co-developed with Claude Code (Anthropic). Experiment scripts were drafted collaboratively with Claude Opus 4.6. The paper draft was initiated with AI assistance and revised by the author. All scientific claims, interpretations, and editorial decisions are the author's own. The use of AI in studying AI-generated conversation archives creates a recursive structure that we consider a feature, not a limitation: the tools being studied are also the tools doing the studying.
