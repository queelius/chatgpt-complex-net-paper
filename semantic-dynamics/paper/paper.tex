...
The central finding is that conversation archives have a dynamical structure independent of their semantic content. Semantic graphs answer ``what is this conversation about?'' Velocity and curvature graphs answer ``how does the conversation move?'' and ``where does it turn?'' These questions have different answers: the NMI between semantic and velocity communities is 0.13--0.21.
...
The episode detection results challenge the standard practice of treating conversations as atomic units. A 1,764-message session is 43 distinct episodes sharing a session container. This connects to Complementary Learning Systems theory~\citep{mcclelland1995complementary,kumaran2016complementary}: episodes (segments) are the hippocampal units, sessions (conversations) are platform artifacts, and trails (links between episodes across sessions) are the neocortical consolidation process.
...
\textbf{No ground truth for episodes.} Episode boundaries are evaluated qualitatively.
