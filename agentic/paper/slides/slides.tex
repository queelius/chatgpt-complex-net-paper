...
\begin{frame}{The Problem: Archives Are Flat. Knowledge Is Not.}
    ...
    \note{%
        ...
        On the left of the slide, what you actually have when you open your
        ChatGPT history: a chronological list. Bayesian inference next to cooking
        pasta next to Solomonoff induction. Adjacency in \emph{time} tells you
        nothing about which conversations are about the same \emph{thing}.\\[2pt]
        On the right, what's hidden. You know structure is there --- you came back
        to Solomonoff three times last month; Bayesian inference is part of a
        domain you've been exploring. That hierarchy is real; it's just invisible
        from a list.\\[2pt]
...
\begin{frame}{Two Ideas from Cognitive Science}
    ...
    \note{%
        ...
        On the left, Complementary Learning Systems. The hippocampus encodes
        individual experiences fast; the neocortex extracts statistical
        ...
        On the right, the structural side. Steyvers and Tenenbaum mapped human
        semantic memory as a network and consistently found small-world topology
        --- dense local clusters plus a few long-range shortcuts. Small-worldness
        scores between roughly 5 and 15.\\[2pt]
        That gives us the hypothesis at the bottom of the slide: if conversation
        archives are an externalised semantic memory, we should see \emph{both}
        signatures.
...
\begin{frame}{Pipeline: Concepts $\rightarrow$ Embeddings $\rightarrow$ Hierarchy}
    ...
    \note{%
        ...
        Following the pipeline from left to right, four stages.\\[2pt]
        ...
        episode. The orange box below the pipeline shows the key instruction in
        the prompt --- specificity. Not ``statistics''; ``Metropolis-Hastings
        algorithm.'' Specific concepts are what makes the geometry work later.\\[2pt]
        ...
        The line at the bottom names the output: a graph linking episodes to
...
\begin{frame}{The Reveal: A Four-Level Hierarchy Emerges}
    ...
    \note{%
        \textbf{Slide 5 -- The Reveal (1:00, $\sim$120w)}\\[2pt]
        \textit{[Pause. Let the picture land.]}\\[2pt]
        Reading down the cascade on the left: 1{,}908 episodes consolidate into 500
        meta-concepts, then 50 themes, then 8 domains. Each level groups roughly
        3.5 of the level below --- the geometric signature of a hierarchy.\\[2pt]
        The table on the right names the eight domains. I didn't pre-specify
        them --- they emerged from the clustering and reflect my actual research
        and project history.\\[2pt]
        Two surprises, in the block below.\\[2pt]
        \emph{First}: there's no clean number of clusters. Knowledge is a
        continuum; eight is a useful summary, not a ``right answer.''\\[2pt]
        \emph{Second}: most episodes belong to several domains at once. We'll
        come back to that.\\[2pt]
...
\begin{frame}{Signature 1: Small-World Topology}
    \begin{columns}[T]
        ...
        \begin{tikzpicture}[scale=0.95, transform shape]
            ...
            \node[font=\footnotesize, gray] at (0, 2.65)
                {499 meta-concept nodes \,$\cdot$\, 5{,}935 co-occurrence edges};
            ...
            % Legend: spell out what a node and an edge are
            \node[font=\scriptsize, align=left] at (0, -2.85) {%
                \begin{tabular}{@{}l@{\;\;}l@{}}
                    \textbf{Node:} & 1 of 500 meta-concepts \\
                    \textbf{Edge:} & both concepts appear in the same episode
                \end{tabular}};
        \end{tikzpicture}
    ...
    \note{%
        \textbf{Slide 6 -- Small-World Topology (45 s, $\sim$95w)}\\[2pt]
        Quick recap: these are the same 500 meta-concepts from the cascade, now
        drawn as a network. A node is one meta-concept. An edge means the two
        showed up in the same conversation. So if one chat covered both Bayesian
        inference and MCMC, those nodes get linked --- even across domains.\\[2pt]
        The cartoon shape --- tight local clusters with a few long-range
        shortcuts crossing the middle --- is the classic small-world
        signature.\\[2pt]
        Clustering comes in roughly seven times higher than random. Path lengths
        are unchanged. Small-worldness lands at 6.6. For comparison: human
        semantic memory networks score between 5 and 15. So we sit right in that
        range.\\[2pt]
        I'm not claiming archives \emph{are} semantic memory --- just that they
        show the same structural fingerprint. Signature one, done.
    }
\end{frame}
...
\begin{frame}{Signature 2: Sublinear Vocabulary Growth}
    ...
    \note{%
        \textbf{Slide 7 -- Vocabulary saturates (30 s, $\sim$70w)}\\[2pt]
        This is Heaps' law: vocabulary grows like $n$ raised to beta. Real text
        usually sits at beta around 0.4 to 0.6 --- and lower means stronger
        saturation.\\[2pt]
        The top curve is raw concepts at beta 0.93 --- nearly linear. The bottom
        curve is meta-concepts at beta 0.32 --- strongly sublinear. Two null
        models both come in below ours, so the saturation is real --- not just
        an artifact of fixing $k$ at 500.
    }
\end{frame}
...
\begin{frame}{77\% of Episodes Span Multiple Domains}
    ...
    \note{%
        \textbf{Slide 8 -- 77\% span multiple domains (40 s, $\sim$90w)}\\[2pt]
        Look at the two episodes shown in the figure. One came from a
        conversation about statistical methods; the other from philosophy of AI
        --- so they're in different domains. But they share five concepts in
        common, and those concepts actually span \emph{four} different domains
        between them.\\[2pt]
        If you used any one-label clustering method --- Louvain, $k$-means,
        BERTopic --- you'd put these in separate buckets and that link would
        just disappear. Set-based clustering keeps them connected. And this
        isn't rare: 77\% of episodes span two or more domains.
    }
\end{frame}
...
\begin{frame}{Who Borrows From Whom? (Concept Flow)}
    ...
    \note{%
        \textbf{Slide 9 -- Who borrows from whom (40 s, $\sim$90w)}\\[2pt]
        The heatmap shows directional concept flow between domains. Reading
        across a row tells you where that domain's concepts flow out to;
        reading down a column tells you what comes in. That asymmetry is
        something an undirected network can't capture.\\[2pt]
        After controlling against a size-preserving null, sharing actually
        comes in \textbf{39\% below random} --- so domains really are
        separate.\\[2pt]
        But one outlier survives that control: LLM Engineering feeds ML and
        Networks at \textbf{2.67 times} the random expectation. That's a real
        targeted dependency, not spillover.
    }
\end{frame}
...
\begin{frame}{The Full Picture}
    ...
    \note{%
        \textbf{Slide 10 -- The Full Picture (20 s, $\sim$40w)}\\[2pt]
        This is the whole archive in one image: 499 concepts in the inner ring,
        all 1{,}908 episodes around the outside, coloured by domain. The point
        isn't to read individual lines --- it's to see that this is structured,
        not random.\\[2pt]
        \textit{[Brief pause.]}
    }
\end{frame}
...
\begin{frame}{Takeaways \& What's Next}
    \begin{columns}[T]
        ...
        \begin{block}{Why it matters}
            ...
            \begin{itemize}\setlength\itemsep{0.35em}
                ...
                \item \textbf{Concept-level retrieval} (Graph~RAG)
        ...
        \begin{alertblock}{Limits \& next}
            ...
            \begin{itemize}\setlength\itemsep{0.25em}
                ...
                \item \textbf{Cluster count} is a design choice
            \end{itemize}
        \end{alertblock}
    \end{columns}
    ...
    \vspace{0.1cm}
    \centering
    \footnotesize \textcolor{PrimaryBlue}{\textbf{Thank you}}

    \note{%
        \textbf{Slide 11 -- Takeaways (45 s, $\sim$95w)}\\[2pt]
        To wrap up, three findings. A four-level hierarchy emerged from this one
        user's archive. Small-world topology and saturating vocabulary --- the
        same signatures cognitive science predicts for human semantic memory.
        And 77\% of episodes span multiple domains, which is why set-based
        clustering captures what one-label methods would lose.\\[2pt]
        The headline is at the bottom of the slide: AI conversation archives are
        structured cognitive artifacts. The structure was there all along; we
        just needed the right lens.\\[2pt]
