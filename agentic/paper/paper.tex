...
($\beta = 0.320$), with growth dynamics distinguishable from random
clustering ($p < 0.001$), indicating genuine categorical structure.
...
spanning December 2022 through April 2025 (29 months). Conversations cover
statistical methodology, software engineering, AI research, R programming,
philosophy, and creative projects. The archive represents a naturalistic record
of sustained AI-assisted knowledge exploration.
...
\texttt{nomic-embed-text} model~\cite{nussbaum2024nomic}, producing
768-dimensional vectors via contrastive learning on a BERT-derived
architecture. Embeddings are L2-normalized to enable
...
\noindent The meta-concept level groups semantically equivalent concepts from
parallel extraction agents (e.g., ``bootstrap confidence intervals'' and ``BCa
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.65\textwidth]{domain_cooccurrence.pdf}
...
ceiling artifact. Using the same alphabetical episode ordering, we construct
a null model by randomly permuting concept-to-cluster assignments (preserving
cluster sizes) and computing $\beta$ for 1,000 permutations. The null distribution has
$\beta_\text{null} = 0.268 \pm 0.006$, and the observed $\beta = 0.320$
lies above the entire null distribution ($p < 0.001$). Semantic clustering
produces a \emph{higher} $\beta$ (less sublinear, closer to linear) than
random assignment: the semantically coherent clusters create meaningful
distinctions, so encountering a genuinely new meta-concept requires exploring
novel intellectual territory rather than merely encountering new vocabulary.
...
raw concepts, $\beta = 0.320$ at the meta-concept level). The shaded band
shows the 95\% null model range under random cluster assignment.
\textbf{Right}: Null model distribution---the observed $\beta = 0.320$ lies
entirely above the null ($\beta_\text{null} = 0.268$, $p < 0.001$),
confirming that semantic structure shapes the growth dynamics.}
\label{fig:heaps}
\end{figure}
...
The null model comparison reveals a more nuanced finding: semantic clustering
produces \emph{higher} $\beta$ (less sublinear) than random cluster assignment
($\beta_\text{null} = 0.268$, $p < 0.001$). This means the semantically
coherent clusters create meaningful categorical distinctions---encountering a
genuinely new meta-concept requires exploring novel intellectual territory,
whereas random clusters generate ``accidental'' consolidation through arbitrary
groupings. Under random assignment, conceptually unrelated items share clusters
...
\paragraph{Small-World Topology.}
The small-world coefficient $\sigma \approx 6.6$, validated against 100
Erd\H{o}s-R\'{e}nyi random graphs~\cite{humphries2008network}, matches human
semantic memory benchmarks~\cite{steyvers2005large}. While we do not claim
...
hierarchies. Validating structural universality---whether sublinear growth
and small-world topology are preserved across users---requires multi-user
datasets. Hierarchy cut points ($k = 500, 50, 8$) are chosen, not
discovered; the silhouette analysis shows generally increasing scores
with $k$ (monotonic for $k \geq 20$, with no distinct peak),
indicating no natural cluster boundaries. Concept extraction
depends on the specific LLM and prompt used; alternative models or prompts
would produce different concept sets. Despite these limitations, the
convergence with human semantic memory benchmarks across multiple metrics
suggests that the structural properties may reflect general features of
categorical knowledge systems rather than idiosyncrasies of this particular
archive.
...
interpretable domains, sublinear vocabulary growth validated against a
random-clustering null model ($p < 0.001$),
