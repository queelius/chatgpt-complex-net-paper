...
Millions of users maintain conversation archives with ChatGPT, Claude, and other large language models, creating externalized records of their cognitive activity~\citep{zhao2023survey}.
...
Following the methodology of~\citet{towell2025cognitive}, we embed each session using nomic-embed-text~\citep{nussbaum2024nomic} (768 dimensions), compute all-pairs cosine similarity, and threshold to construct episodic similarity networks.
...
Community detection via the Louvain algorithm~\citep{blondel2008fast} identifies 15 ChatGPT communities (e.g., ML/AI, Statistics, Philosophy, Web Development) and 12 agentic communities (mapped to coding projects rather than knowledge domains).
...
For each community, the top 20 sessions by message count are presented to the LLM with a prompt requesting 2--5 abstract concepts that characterize the community's cognitive content.
...
Each concept is represented as ``name: description'' and embedded via nomic-embed-text into the same 768-dimensional space used for episodes.
...
\begin{itemize}
  ...
  \item \textbf{Percolation analysis}: Threshold sweep from $\theta = 0.45$ to $0.95$, tracking giant component fraction and platform composition~\citep{stauffer1994introduction}.
  ...
  \item \textbf{Link prediction}: Logistic regression on topological features (Adamic-Adar~\citep{adamic2003friends}, common neighbors, Jaccard index, degree sum)~\citep{lu2011link}.
  ...
  \item \textbf{Null models}: Erd\H{o}s-R\'enyi (same density), configuration model (same degree sequence), geometric random graph (same edge count)---100 realizations each.
...
Triangle census provides a motif-level view of the rebar function.
...
The innermost core ($k = 19$, 31 nodes, density $0.82$) contains 19 ChatGPT and 12 agentic concepts---a $61$:$39$ ratio, compared to the full network's $69$:$31$.
...
Bridging is distributed across 37 satellite connectors rather than concentrated in a few hubs---a leaderless, egalitarian architecture.
...
\begin{itemize}
  ...
  \item \textbf{Network class}: Clustering falls between Erd\H{o}s-R\'enyi ($z = +60$) and geometric random graphs ($z = -9.4$), placing the concept network in the ``small-world semantic space'' regime---more structured than random but less constrained than geometry.
\end{itemize}
...
This extends the ``extended mind'' thesis~\citep{clark1998extended}: not only do external AI tools function as cognitive extensions, but the \emph{conceptual structures} they help construct converge across tools.
...
This has implications for the study of multiplex networks more broadly~\citep{boccaletti2014structure,kivela2014multilayer}.
